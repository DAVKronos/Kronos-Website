...
import { getAliases } from "../Admin/queries";
...
const mailingListFields = [{
    ...
    required: false,
    multiple: true,
    itemQuery: commissionId => commissionId ? [['commissions', commissionId, 'users'], getUsersForCommission] : [['users'],getUsers]
}]

// TODO: make required do something (with react-hook-form)
const MailingListForm = ({values, setValue, children }) => {
    return <Form>
        {mailingListFields.map(({name, type, required, itemQuery, ...otherProps}) => {
            let newItemQuery = name === 'user_ids' ? itemQuery(values['commission_id']) : itemQuery;
            return <FormField
                {...otherProps}
                key={name}
